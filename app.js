...
// Complete the exercise in the space below:
inventorNamedAda = inventors.find(({ name }) => name === "Ada");
...
firstLast = people.map((person) => {
    let p = person.split(', ')  // split the string into an array at the ,
    console.log(p)
    return `${p[1]} ${p[0]}` // grabbed the element at index of 1 and then grabbed the element at index 0
})
